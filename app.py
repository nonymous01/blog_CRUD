...
def ajouter():
     ajout=img.query.all()
     cpt = session.get('cpt', 0)  # Récupérez la valeur de cpt à partir de la session
     return render_template('accueil.html',ajout=ajout, cpt=cpt)
...
def connect():
    cpt = session.get('cpt', 0)  # Récupérez la valeur de cpt à partir de la session
    return render_template("accueil.html", cpt=cpt)
